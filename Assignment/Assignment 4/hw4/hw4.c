...
void to_image(int ** input,unsigned char ** output,int ROW,int COLUMN);
...
void to_image(int ** input,unsigned char** output,int ROW,int COLUMN){
    ...
    for(int i=0;i<ROW;i++){
        for(int j=0;j<COLUMN;j++){
            max=max> *(*(input+ROW)+COLUMN)?max:*(*(input+ROW)+COLUMN);
        }
    }
    ...
    for (int i = 0; i < ROWS; i++) {
        for (int j = 0; j < COLUMNS; j++) {
            ...
            *(*(output+ROW)+COLUMN)=(unsigned char)(norm*(float)abs(*(*(input+ROW)+COLUMN)));
        }
    }
